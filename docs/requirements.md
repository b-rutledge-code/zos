...
8. **Mood** – Each non-empty Zork command the player submits reduces boredom and unhappiness via `player:getStats():remove(CharacterStat.BOREDOM, …)` / `CharacterStat.UNHAPPINESS`. Relief is earned only on input (not CRT open-time). A rolling real-time cap limits total relief per minute so command spam cannot dump mood instantly. ZOS shell commands (`dir`, `a:`, etc.) do not apply mood.
9. **Moment XP** – Curated Zork story beats grant VHS-style perk XP (`addXp(player, perk, 50 * amount)`, amount 1 or 2), honoring `SandboxVars.LevelForMediaXPCutoff`. Each moment id is claimed once on the character (`ModData.zosXpClaimed.<titleCmd>.<id>`), so RESTART / replay cannot re-grant the same beat. Detectors use room name, score thresholds / score-up, and output substrings after each Zork turn. Shell commands do not grant XP.
